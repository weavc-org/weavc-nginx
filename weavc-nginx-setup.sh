...
if [[ ! -v "$SSL_GENERATE" ]] && [ "$SSL_GENERATE" == "yes" ]; then
    openssl req -newkey rsa:$KEY_SIZE -new -nodes -x509 -days 3650 -keyout /defaults/run/secrets/privkey.pem -out /defaults/run/secrets/fullchain.pem -subj /C=$SSL_C/ST=$SSL_ST/L=$SSL_L/O=$SSL_O/OU=$SSL_OU/CN=$SSL_CN
fi
...
if [[ ! -v "$DH_GENERATE" ]] && [ "$DH_GENERATE" == "yes" ]; then
    openssl dhparam -out /defaults/run/secrets/dhparam.pem $KEY_SIZE
fi

ls -al /run/secrets
ls -al /defaults/run/secrets
